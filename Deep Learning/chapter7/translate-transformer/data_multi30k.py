...
if __name__ == "__main__":
    # 用了解析器后可以直接在命令行传参，而不用在python中传参,这样就不用每次改 Python 源码里的固定值了
    # 本质上还是“传参给 Python 程序”，只是入口从“代码里写死”变成了“命令行输入”。
    ...
    for mode in ["train", "val", "test"]:
        moses_cut(
            ...
            local_data_path / f"{mode}_src.cut.txt",
            lang=args.src_lang,
        )
        print(f"[{mode}] 源语言文本分词完成")
        moses_cut(
            local_data_path / f"{mode}.{args.trg_lang}", # 读取目标语言文件
            local_data_path / f"{mode}_trg.cut.txt",
            lang=args.trg_lang,
        )
